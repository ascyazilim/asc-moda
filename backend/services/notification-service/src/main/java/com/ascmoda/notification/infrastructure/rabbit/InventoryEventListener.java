...
public class InventoryEventListener {
    ...
    @RabbitListener(queues = RabbitEventTopology.NOTIFICATION_INVENTORY_QUEUE)
    public void onMessage(String message) {
        try {
            notificationService.process(message);
        } catch (InvalidMessagePayloadException | UnsupportedNotificationEventException ex) {
            log.warn("Skipped invalid inventory notification event message reason={}", ex.getMessage());
        } catch (RuntimeException ex) {
            log.error("Unexpected inventory notification event processing failure", ex);
            throw ex;
        }
    }
}
