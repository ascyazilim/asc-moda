...
export const cartApi = {
  ...
  async getSummary(customerId: string): Promise<CartSummary> {
    const response = await apiClient.get<CartSummaryDto>(`/api/v1/carts/${customerId}/summary`);

    return mapCartSummary(response.data);
  },

  async addItem(payload: AddCartItemPayload, customerId: string): Promise<Cart> {
  ...
  async removeItem(itemId: string, customerId: string): Promise<Cart> {
